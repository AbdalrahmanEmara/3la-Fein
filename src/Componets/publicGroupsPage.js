import PublicGroupsCards from "./discoverCards/publicGroupsCards";
...
function PublicGroupsPage(groups){
    const [groupsLength, setGroupsLength] = useState(Math.ceil(groups.groups.length/8));
    ...
    return(
        <div style={{width: '77%', height: 'auto', margin: 'auto'}}>
            ...
            <p style={{margin: '0px', width: '100%'}}>
                <span style={{font: '500 15px "Poppins"', color: '#777E90'}}>Get in touch </span>
                <span style={{font: '600 20px "Poppins"'}}>with people</span>
            </p>

            <div className={Style.publicGroupsPageDiv}>
                <p style={{width: '100%', font: '500 16px "Poppins"', color: '#777E90', margin: '0px'}}>There are 100+ groups you can join</p>

                {groupsBatch.map((group) => <PublicGroupsCards image={group.image}
                    name={group.name}
            ...
            <div className={Style.arrowButtons}>
                <div onClick={moveBackward} className={c>8? `${Style.arrowBig}` : ''}>
                ...
                <div onClick={moveForward} className={c/8 !== groupsLength? `${Style.arrowBig}` : ''}>
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
