...
function PublicGroupsCards(publicGroups) {
  ...
  const userJoined = currentUser?.history?.some(
    (item) => item.name === publicGroups.name
  );

  // Increase currentMembers by 1 if user joined
  const displayedMembers = userJoined
    ? parseInt(publicGroups.currentMembers, 10) + 1
    : parseInt(publicGroups.currentMembers, 10);
  ...
  return (
    <div className={Style.publicGroupsCards}>
      <Link to="/">
        <img className={Style.groupImage} src={publicGroups.image} />
      </Link>
      <div className={Style.publicGroupsCardsDiv}>
        <div className={Style.publicGroupsCardsDiv1}>
          <p style={{ font: '500 16px "Poppins"', margin: "8px 0px" }}>
            {publicGroups.name}
          </p>
          ...
          >
            {publicGroups.category}
          </p>

          <div
            className="d-flex align-items-center"
            style={{ height: "20px", margin: "8px 0px" }}
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="12"
          ...
          >
            {userJoined ? "joined" : "join with us"}
          </button>
        </div>
        ...
        <div className={Style.publicGroupsCardsDiv2}>
          <Link to="/">
            <img src={publicGroups.adminPic} />
          </Link>

          <p
            className="mx-auto"
            style={{ margin: "0px", font: '400 12px "Poppins"' }}
          >
            {publicGroups.adminName}
          </p>
          ...
          >
            {displayedMembers} / {publicGroups.maxMembers}
          </p>
        </div>
      </div>
    </div>
