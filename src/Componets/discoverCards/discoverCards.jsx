...
function DiscoverCards(place) {
  ...
  const changeColor = (e) => {
    ...
    if (isFavorite(place.name)) {
      ...
      removeFavoriteFromCurrentUser(place.name); // you need to implement this
      setHeartColor("#B1B5C4");
    } else {
      // add to favorites
      addFavoriteToCurrentUser(place); // your add function
  ...
  return (
    <div
      className={`${Style.discoverCards}`}
      onClick={() => {
        if (place.onCardClick) {
          place.onCardClick();
        }
      }}
    >
      ...
      <div className="d-flex" style={{ height: "20px" }}>
        {typeof place.isOpen === "boolean" ? (
          ...
          <button className={Style.detailButton} style={{ marginLeft: "auto" }}>
            Details
          </button>
        )}
      </div>
